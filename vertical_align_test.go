...
func applyVerticalAlign(t *testing.T, css string) *FormattingStyles {
	t.Helper()
	...
	if err := StylesToStyles(ih, resolveCSSText(css), fe, parentSize); err != nil {
...
func TestVerticalAlignScriptShiftFromParentSize(t *testing.T) {
	...
	sup := applyVerticalAlign(t, "font-size: 58%; vertical-align: super")
	if want := parent / 3; sup.yoffset != want {
		t.Errorf("super: yoffset = %s, want %s (parent em / 3)", sup.yoffset, want)
	}

	sub := applyVerticalAlign(t, "font-size: 58%; vertical-align: sub")
...
func TestVerticalAlignLengthAndPercentage(t *testing.T) {
	em := applyVerticalAlign(t, "font-size: 58%; vertical-align: 0.4em")
	...
	pt := applyVerticalAlign(t, "vertical-align: -2pt")
	...
	if err := StylesToStyles(pct, resolveCSSText("vertical-align: 50%"), fe, pct.Fontsize); err != nil {
		t.Fatalf("StylesToStyles: %v", err)
	}
...
func TestVerticalAlignNestedScriptsStack(t *testing.T) {
	...
	outer := body.Clone()
	if err := StylesToStyles(outer, resolveCSSText("font-size: 58%; vertical-align: super"), fe, body.Fontsize); err != nil {
	...
	inner := outer.Clone()
	if err := StylesToStyles(inner, resolveCSSText("vertical-align: super"), fe, outer.Fontsize); err != nil {
